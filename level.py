...
def create_level(level_num, powerup_manager):
    ...
    return items, item_speeds, item_directions, obstacles, obstacle_speeds, obstacle_directions, health_items, powerups, powerup_speeds, powerup_directions, "Level criado"
...
# def create_obstacle(level_num):
#     num_obstacles = 5 + level_num  # Incrementa a quantidade de obstáculos com o nível
#     obstacles = [pygame.Rect(random.randint(0, screen_width - 20), random.randint(100, screen_height - 120), 20, 20) for _ in range(num_obstacles)]
#     obstacle_speeds = [random.uniform(1.0, 6.0) + 0.75 * level_num for _ in range(len(obstacles))]  # Velocidade inicial mais aleatória e aumentada com o nível
